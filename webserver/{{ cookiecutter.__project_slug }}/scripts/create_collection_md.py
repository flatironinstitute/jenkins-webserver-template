...
def check_single_source(sites=glob('{{ cookiecutter.push_target_dir }}/*/')):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--subdir_index", choices=["redirect", "browse"],
                        help="Whether the subdirectory redirects (e.g., {{ cookiecutter.push_target_dir }}/index.html) will redirect or allow users to browse.")
    args = vars(parser.parse_args())
    if check_single_source():
        sites = ['{{ cookiecutter.push_target_dir }}/']
    else:
        sites = glob('{{ cookiecutter.push_target_dir }}/*/')
    for site in sites:
    ...
    if args['subdir_index'] == 'redirect':
        with open(op.join('{{ cookiecutter.push_target_dir }}', 'index.html'), 'w') as f:
            f.write(REDIRECT_HTML.format('../'))
    elif args['subdir_index'] == 'browse':
        if op.exists(op.join('site', 'browse_pages', '{{ cookiecutter.push_target_dir }}')):
            ...
            with open(op.join('site', 'browse_pages', '{{ cookiecutter.push_target_dir }}', 'index.md'), 'w') as f:
    ...
    if args['root_index_redirect']:
        try:
            ...
            raise RuntimeError("{{ cookiecutter.push_target_dir }} does not contain any contents, so cannot redirect from root index!")
        with open(op.join('site', 'index.html'), 'w') as f:
